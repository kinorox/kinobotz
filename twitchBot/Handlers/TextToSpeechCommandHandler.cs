...
namespace twitchBot.Handlers
{
    public class TextToSpeechCommandHandler : BaseCommandHandler<TextToSpeechCommand>
    {
        ...
        private readonly AudioHub audioHub;

        public TextToSpeechCommandHandler(ElevenLabsClient elevenLabsClient, IRedisClient redisClient, IConfiguration configuration, AudioHub audioHub)
        {
        ...
        public override async Task<Response> InternalHandle(TextToSpeechCommand request, CancellationToken cancellationToken)
        {
            ...
            var discordWebhookClient = new DiscordWebhookClient(configuration["DISCORD_WEBHOOK"]);
            var fileName = $"{request.Voice}-{DateTime.Now}-{(request.Message.Length > 10 ? request.Message[..10] : request.Message)}.mp3";
            await discordWebhookClient.SendFileAsync(stream: new MemoryStream(audioStreamData), fileName, $"Audio generated by {request.Username} on {request.Channel} at {DateTime.Now}. Audio transcription: {request.Message}");

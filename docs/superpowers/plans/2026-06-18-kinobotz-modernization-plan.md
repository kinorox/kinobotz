...
- **[NEW - do FIRST] UI deploy cutover (monorepo -> kinobotz-ui app).** Today only the backend deploys on merge to `main`; the live UI still deploys from the standalone `github.com/kinorox/kinobotz-ui` repo. Add `.github/workflows/frontend-deploy.yml` (push to `main`, path-filtered `frontend/**`) that builds `frontend/Dockerfile` and runs `heroku container:push web -a kinobotz-ui` + `heroku container:release web -a kinobotz-ui` (auth via `HEROKU_API_KEY` secret). This must land BEFORE the Vite/Pinia/TS changes below so they actually ship. After a monorepo-sourced deploy is verified live at the UI URL, archive the standalone `kinobotz-ui` repo so the monorepo is the single source of truth.
